def bmvj_compress(data: bytes) -> bytes:
    def make_tree(x):
        ...
        w = sorted(work, key=lambda a: sum(x[i] for i in a))[:(len(x)-x.count(0)-1)*2]
        y = sum(w, [])
        z = [y.count(i) for i in range(len(x))]
        c, l = 0, 0
        for i in sorted(range(len(x)), key=z.__getitem__):
            if not x[i]:
                z[i] = ""
                continue
            c <<= (z[i] - l)
    ...
    #print(lens_tree, syms_tree, offs_tree)
    ...
    for sym, off in zip(syms, offs):
        ...
        next_bin = next_bin[len(next_bin)&~7:] + syms_tree[sym]
        #print(syms_tree[sym], off, f"{sym:02X}")
        if off is not None:
            if off < 2:
